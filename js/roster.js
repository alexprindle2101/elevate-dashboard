      ...
      if (pending.length > 0) {
        ...
        pendingList.innerHTML = pending.map(name => `
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
            ...
            <button onclick="App.approveUnlock('${name}')" style="background:rgba(34,197,94,0.13);border:1px solid rgba(34,197,94,0.33);border-radius:6px;color:#22c55e;padding:3px 10px;font-family:'Barlow Condensed',sans-serif;font-size:11px;font-weight:700;letter-spacing:1px;cursor:pointer;text-transform:uppercase">Approve</button>
            <button onclick="App.denyUnlock('${name}')" style="background:rgba(229,53,53,0.1);border:1px solid rgba(229,53,53,0.3);border-radius:6px;color:#e53535;padding:3px 10px;font-family:'Barlow Condensed',sans-serif;font-size:11px;font-weight:700;letter-spacing:1px;cursor:pointer;text-transform:uppercase">Deny</button>
          </div>`).join('');
      } else {
        ...
        <td style="padding:12px 16px">
          <div id="roster-display-${safeEmail}" style="display:flex;align-items:center;gap:8px">
            ...
            <button onclick="Roster.startEditPerson('${safeEmail}')" title="Edit name & email"
              style="background:none;border:1px solid rgba(26,92,229,0.2);border-radius:6px;padding:3px 7px;cursor:pointer;font-size:12px;color:var(--silver-dim);line-height:1">✏️</button>
          </div>
          <div id="roster-edit-${safeEmail}" style="display:none">
            <input id="roster-edit-name-${safeEmail}" type="text" value="${p.name.replace(/"/g, '&quot;')}"
              style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.6);border:1px solid rgba(26,92,229,0.3);border-radius:6px;padding:5px 8px;color:var(--white);font-family:'Barlow Condensed',sans-serif;font-size:14px;font-weight:700;outline:none;margin-bottom:4px">
            <input id="roster-edit-email-${safeEmail}" type="email" value="${email}"
              style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.6);border:1px solid rgba(26,92,229,0.3);border-radius:6px;padding:4px 8px;color:var(--white);font-family:'Barlow Condensed',sans-serif;font-size:11px;outline:none;margin-bottom:6px">
            <div style="display:flex;gap:6px">
              ...
              <button onclick="Roster.cancelEditPerson('${safeEmail}')"
                style="background:rgba(0,0,0,0.05);border:1px solid rgba(26,92,229,0.2);border-radius:5px;padding:3px 10px;color:var(--silver);font-family:'Barlow Condensed',sans-serif;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;cursor:pointer">Cancel</button>
            </div>
          </div>
        </td>
        <td style="padding:12px 16px">
          <select onchange="App.setPersonRole('${safeName}',this.value)"
            style="background:rgba(0,0,0,0.06);border:1px solid rgba(26,92,229,0.25);border-radius:6px;color:var(--white);padding:5px 8px;font-family:'Barlow Condensed',sans-serif;font-size:12px;cursor:pointer;outline:none">
            ${roleSelect}
          </select>
        </td>
        <td style="padding:12px 16px">
